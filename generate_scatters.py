...
<body>
  ...
  <script>
    ...
    // ORDINAL, no categórico: turquesa = más libre, rojo = menos libre.
    // Coincide con el mapa mundial y con QC de generate_charts.py. Estaba
    // INVERTIDO (cuartil 1 en rojo), que contradecía al mapa y al sombreado.
    const Q_COLORS = {{ 1:'#0A9396', 2:'#94D2BD', 3:'#EE9B00', 4:'#C71E1D' }};
    ...
    function buildChart(){{
      ...
      const gridColor = isDk ? '#1A2940' : '#F1EDE5';
      const axisColor = isDk ? '#2A3A50' : '#E2DDD3';
      const labelColor = isDk ? '#64748B' : '#64748B';
      const nameColor = isDk ? '#64748B' : '#64748B';
      const mc = isDk ? '#64748B' : '#64748B';

      // Sombreado de fondo por tramo de libertad. MISMA escala que las burbujas y
      // que el mapa: rojo abajo, turquesa arriba. Estaba en rgba() de Tailwind, que
      // el barrido por hexadecimales no alcanzó.
      //   #C71E1D=199,30,29 · #EE9B00=238,155,0 · #94D2BD=148,210,189 · #0A9396=10,147,150
      const qBands = [
        [{{xAxis:3.5}},{{xAxis:5.5, itemStyle:{{color:isDk?'rgba(199,30,29,.07)':'rgba(199,30,29,.05)'}}}}],
        [{{xAxis:5.5}},{{xAxis:6.5, itemStyle:{{color:isDk?'rgba(238,155,0,.07)':'rgba(238,155,0,.05)'}}}}],
        [{{xAxis:6.5}},{{xAxis:7.5, itemStyle:{{color:isDk?'rgba(148,210,189,.09)':'rgba(148,210,189,.07)'}}}}],
        [{{xAxis:7.5}},{{xAxis:9.2, itemStyle:{{color:isDk?'rgba(10,147,150,.07)':'rgba(10,147,150,.05)'}}}}]
      ];
      ...
      chart.setOption({{
        ...
        tooltip:{{
          ...
          backgroundColor:isDk?'#1A2940':'#fff',
          borderColor:isDk?'#2A3A50':'#E2E8F0',
          ...
          formatter: p => {{
            ...
            return `<div style="min-width:190px">`
              ...
              +`<div style="font-size:10px;color:${{mc}};margin-top:4px;border-top:1px solid ${{isDk?'#2A3A50':'#E2E8F0'}};padding-top:4px">${{R_SHORT[reg]||reg}}</div></div>`;
          }}
        }},
        series:[
